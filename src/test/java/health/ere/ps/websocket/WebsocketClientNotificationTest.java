...
public class WebsocketClientNotificationTest {
    ...
    public void test_Successful_Sending_Of_Backend_Notification() {
        try (Session session = ContainerProvider.getWebSocketContainer().connectToServer(Client.class, uri)) {
            ...
            ereLogger.infov("Log notification event was published for simple log message [{0}] " +
                    "and context list [{1}]", SIMPLE_LOG_MSG, CTX_LIST);

            // Await expected positive receipt confirmation of notification event payload from FE
            // client simulator.
            Assertions.assertEquals(OK_TEST_MESSAGE, StringUtils.defaultString(MESSAGES.poll(10,
                    TimeUnit.SECONDS)));
        ...
        void message(String jsonString) {
            if (StringUtils.isNotBlank(jsonString)) {
                try (JsonReader jsonReader = Json.createReader(new StringReader(jsonString))) {
                    ...
                    String payloadType = StringUtils.defaultString(jsonObject.getString("type"));
                    ...
                    String simpleLogMessage = StringUtils.defaultString(payloadJson.getString(
                            "simpleLogMessage"));
                    String status = StringUtils.defaultString(payloadJson.getString("status"));
                    ...
                    ereLogger.infof("Payload type received: %s", payloadType);
