...
public class ERezeptWorkflowResourceTest {
    ...
    public void testReadHeader() {
        ...
        when(httpServletRequest.getHeader(eq("X-clientCertificate"))).thenReturn(
        ...
        when(httpServletRequest.getHeaderNames()).thenAnswer(new Answer<>() {
            ...
            public Object answer(InvocationOnMock invocation) throws Throwable {
                return Collections.enumeration(Arrays.asList("X-eHBAHandle", "X-SMCBHandle", "X-connectorBaseURL",
                        "X-basicAuthUsername", "X-basicAuthPassword",
                        "X-clientCertificate", "X-clientCertificatePassword", "X-clientSystemId", "X-mandantId",
                        "X-tvMode", "X-userId", "X-version",
                        "X-workplaceId"));
            }
        ...
        assertEquals("data:application/x-pkcs12;base64,MIA",
                runtimeConfig.getConfigurations().getClientCertificate().substring(0, 36));
