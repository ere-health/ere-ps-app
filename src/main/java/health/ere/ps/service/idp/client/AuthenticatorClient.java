...
public class AuthenticatorClient {
    ...
    public AuthorizationResponse doAuthorizationRequest(
        ...
        IdpHttpClientService idpHttpClientService = getIdpHttpClientInstanceByUrl(authorizationRequest.getLink());
        try (Response response = idpHttpClientService.doAuthorizationRequest(
            scope,
            "code",
            ...
            JsonObject userConsent = jsonObject.getJsonObject("user_consent");
            Map<String, String> requestedScopes = toMap(userConsent.getJsonObject("requested_scopes"));
            Map<String, String> requestedClaims = toMap(userConsent.getJsonObject("requested_claims"));
            AuthenticationChallenge challenge = new AuthenticationChallenge(
                new JsonWebToken(jsonObject.getString("challenge")),
                new UserConsent(requestedScopes, requestedClaims)
            );
            return new AuthorizationResponse.AuthorizationResponseBuilder().authenticationChallenge(challenge).build();
        }
    }
    ...
    private void checkResponseForErrorsAndThrowIfAny(
        final Response loginResponse
    ) throws IdpException, IdpClientException {
        if (loginResponse.getStatus() == 302) {
            checkForForwardingExceptionAndThrowIfPresent((String) loginResponse.getHeaders().getFirst("Location"));
        }
        if (loginResponse.getStatus() / 100 == 4) {
            throw new IdpClientException(
                "Unexpected Server-Response: " + loginResponse.getStatus() + " " + loginResponse.readEntity(String.class)
            );
        }
    }
    ...
    private void checkForForwardingExceptionAndThrowIfPresent(final String location)
        throws IdpException {
        UriUtils.extractParameterValueOptional(location, "error").ifPresent(
            Errors.rethrow().wrap((Throwing.Consumer<String>) errorCode -> {
                Optional<String> gematikCode = UriUtils.extractParameterValueOptional(location, "gematik_code");
                String errorDescription = UriUtils.extractParameterValueOptional(location, "error_description").orElse(errorCode);
                String msg = "Server-Error with message: " + gematikCode.map(code -> code + ": ").orElse("") + errorDescription;
                throw new IdpClientException(msg);
            }));
    }
    ...
    public AuthenticationResponse performAuthenticationWithSsoToken(
        ...
        try (Response response = idpHttpClientService.doAuthenticationRequestWithSsoToken(
    ...
    public IdpTokenResult retrieveAccessToken(
        ...
        IdpHttpClientService idpHttpClientService = getIdpHttpClientInstanceByUrl(tokenRequest.getTokenUrl());

        try (Response response = idpHttpClientService.doAccessTokenRequest(
            "authorization_code",
            tokenRequest.getClientId(),
            tokenRequest.getCode(),
            keyVerifierToken.getRawString(),
            tokenRequest.getRedirectUrl())
        ) {
            checkResponseForErrorsAndThrowIfAny(response);
    ...
    private IdpJwe buildKeyVerifierToken(
        final byte[] tokenKeyBytes,
        final String codeVerifier,
        final PublicKey idpEnc
    ) {
        final JwtClaims claims = new JwtClaims();
        String bytesStr = new String(Base64.getUrlEncoder().withoutPadding().encode(tokenKeyBytes));
        claims.setStringClaim(TOKEN_KEY.getJoseName(), bytesStr);
    ...
    public DiscoveryDocumentResponse retrieveDiscoveryDocument(final String discoveryDocumentUrl)
        ...
        try (Response response = idpHttpClientService.doGenericGetRequest()) {
            ...
            Map<String, Object> discoveryClaims = TokenClaimExtraction.extractClaimsFromJwtBody(token);
    ...
    public IdpHttpClientService getIdpHttpClientInstanceByUrl(String url) throws IdpClientException {
        try {
